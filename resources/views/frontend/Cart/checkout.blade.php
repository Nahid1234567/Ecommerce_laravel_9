				...
				<div class="col-lg-4" >
					<div class="card">
						<div class="pl-4 pt-2">
							...
							@php 
							  $data= DB::table('coupons')->where('status',"Active")->get();
							@endphp
							@foreach($data as $row)
							<p style="color: black;">coupon:{{ $row->coupon_code}} <span style="float: right; padding-right: 5px; color: green;">Discount: {{ $row->coupon_amount}}{{ $setting->currency }}</p>
							@endforeach			
							{{-- coupon apply --}}
